...
from app import db
from app.models import ReportTemplate, User, Form
...
class ReportTemplateController:
    ...
    def create_template(data: Dict[str, Any], user: User) -> Tuple[Optional[Dict[str, Any]], Optional[str], int]:
        ...
        Args:
            data (dict): Data for the new template (name, description, form_id, configuration, is_public).
        ...
        form_id = data.get('form_id')  # Changed from user_id to form_id
        ...
        if not form_id:  # Validate form_id
            logger.warning("Create template failed: Missing form_id.")
            return None, "Missing required field: form_id", 400

        # Verify the form exists and user has permission to create a template for it
        try:
            form = Form.query.filter_by(id=form_id, is_deleted=False).first()
            if not form:
                logger.warning(f"Create template failed: Form ID {form_id} not found.")
                return None, f"Form with ID {form_id} not found", 404

            # Check if user has access to this form (owner or admin)
            is_form_owner = form.user_id == user.id
            is_admin = user.role and user.role.name == RoleType.ADMIN

            if not is_form_owner and not is_admin and not form.is_public:
                logger.warning(f"User '{user.username}' permission denied for creating template for form {form_id}.")
                return None, "Permission denied to create template for this form", 403

            # --- Create and Save Template ---
            new_template = ReportTemplate(
                name=name.strip(),
                description=description.strip() if description else None,
                form_id=form_id,  # Associate with the specified form
                configuration=configuration,  # Store the JSON config
                is_public=is_public
            )
            db.session.add(new_template)
            db.session.commit()
            logger.info(f"Report template '{new_template.name}' (ID: {new_template.id}) created successfully by user '{user.username}' for form {form_id}.")
    ...
    def list_templates(user: User) -> Tuple[List[Dict[str, Any]], Optional[str], int]:
        """
        Lists report templates accessible to the user (templates for forms owned by user + public templates).
    ...
    def get_template(template_id: int, user: User) -> Tuple[Optional[Dict[str, Any]], Optional[str], int]:
        ...
        try:
            ...
            # --- Permission Check ---
            # User must own the form linked to the template, OR the template must be public, OR the user must be an admin.
            is_owner = template.form and template.form.user_id == user.id
    ...
    @staticmethod
    def get_templates_by_form(form_id: int, user: User) -> Tuple[Optional[List[Dict[str, Any]]], Optional[str], int]:
        """
        Get report templates associated with a specific form.
        ...
        Returns:
            Tuple[List[Dict], str, int]: List of templates, error message, status code
        """
        logger.info(f"User '{user.username}' requesting report templates for form ID: {form_id}")
        try:
            # Verify form exists
            form = Form.query.filter_by(id=form_id, is_deleted=False).first()
            if not form:
                logger.warning(f"Form ID {form_id} not found for user '{user.username}'")
                return None, "Form not found", 404
            ...
            if not is_form_owner and not is_public and not is_admin:
                logger.warning(f"User '{user.username}' permission denied for form ID {form_id}")
                return None, "Permission denied to access this form's templates", 403
                
            # Get templates for this form
            ...
            template_list = [template.to_dict_basic() for template in templates]
            logger.debug(f"Found {len(template_list)} templates for form ID {form_id}")
            
            return template_list, None, 200
            
        except Exception as e:
            logger.exception(f"Error retrieving templates for form ID {form_id}: {e}")
            return None, f"An internal error occurred while retrieving templates", 500

    @staticmethod
    def update_template(template_id: int, data: Dict[str, Any], user: User) -> Tuple[Optional[Dict[str, Any]], Optional[str], int]:
        """
        Updates an existing report template if the user has permission (form owner or admin).

        Args:
            template_id (int): The ID of the template to update.
            data (dict): Data containing fields to update (name, description, configuration, is_public, form_id).
        ...
        try:
            ...
            # --- Permission Check ---
            # Only the form owner or an admin can update a template.
            is_owner = template.form and template.form.user_id == user.id
            is_admin = user.role and user.role.name == RoleType.ADMIN
            ...
            if 'is_public' in data:
                if isinstance(data['is_public'], bool):
                    ...
                    return None, "Invalid value for is_public (must be true or false)", 400
            if 'form_id' in data:  # Handle form_id update if provided
                form_id = data['form_id']
                if not isinstance(form_id, int) or form_id <= 0:
                    return None, "Invalid value for form_id", 400
                ...
                # Check if user has access to the new form
                is_form_owner = form.user_id == user.id
                if not is_form_owner and not is_admin and not form.is_public:
                    return None, "Permission denied to associate template with this form", 403
    ...
    def delete_template(template_id: int, user: User) -> Tuple[Optional[str], int]:
        """
        Soft-deletes a report template if the user has permission (form owner or admin).
        ...
        try:
            ...
            # --- Permission Check ---
            # Only the form owner or an admin can delete a template.
            is_owner = template.form and template.form.user_id == user.id
            is_admin = user.role and user.role.name == RoleType.ADMIN
