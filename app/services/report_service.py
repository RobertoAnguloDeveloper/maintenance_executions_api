        ...
        with xlsxwriter.Workbook(output, {'in_memory': True, 'remove_timezone': True, 'strings_to_numbers': False, 'strings_to_formulas': False}) as workbook:
            ...
            for report_type, result in processed_data.items():
                ...
                try:
                    ...
                    # This inner try MUST be indented
                    try:
                        error_sheet_name = f"ERROR_{sheet_name[:25]}"
                        # This if MUST be indented further
                        if error_sheet_name not in workbook.sheetnames: # <--- Around Line 973
                            error_worksheet = workbook.add_worksheet(error_sheet_name)
                            error_worksheet.write(0, 0, f"Failed to write sheet '{sheet_name}':")
                            error_worksheet.write(1, 0, str(sheet_err))
                    # This except MUST be at the same indentation level as the inner try
                    except Exception as inner_err:
                        logger.error(f"Could not write error sheet for {sheet_name} either: {inner_err}")
                        pass
